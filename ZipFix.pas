...
type
  ...
  private
    ...
    procedure DoZipFix;
  public
    procedure Execute(InStream: TMemoryStream;
    OutStream: TStream); {$IFDEF VER130_PLUS} overload;{$ENDIF}
    {$IFDEF VER130_PLUS}
    procedure Execute(const InFilename, OutFilename: string); overload;
    {$ENDIF}
  published
...
*)
  ...
  //array of TCentralFileHeaders constitute the Central directory...
...
//---------------------------------------------------------------------
  ...
  pFileInfo = ^TFileInfo;         //first 42 bytes are almost identical to the
  TFileInfo = packed record       //CentralFileHeader record but without HeaderSig
    MadeByVersion      : Byte;    //(1)
    HostVersionNo      : Byte;    //(1)
    Version            : Word;    //(2)
    Flag               : Word;    //(2)
...
begin
  ...
  for i := 0 to fFileList.count-1 do
    ...
    begin
      ...
      //i'm not sure if the Central Directory ExtraField is different
      //to the local ExtraField but zero this out for the Central Directory.
      ExtraFieldLength := 0;
    end;
